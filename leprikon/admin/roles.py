...
class ManagerAdmin(SendMessageAdminMixin, admin.ModelAdmin):
    ...
    def add_school_year(self, request, queryset):
        ...
        class SchoolYearForm(forms.Form):
            school_year = forms.ModelChoiceField(
                ...
                help_text=_('All selected managers will be added to selected school year.'),
        ...
        if request.POST.get('post', 'no') == 'yes':
            ...
            if form.is_valid():
                ...
                for manager in queryset.all():
                    manager.school_years.add(school_year)
                self.message_user(request, _('Selected managers were added to school year {}.').format(school_year))
                return
        else:
            form = SchoolYearForm()
    ...
    add_school_year.short_description = _('Add selected managers to another school year')
    ...
    def courses_link(self, obj):
        return '<a href="{url}?managers__id={manager}">{count}</a>'.format(
