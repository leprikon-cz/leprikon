...
def time_slot_format(start_time: time, end_time: time) -> str:
    return _("{start} - {end}").format(start=start_time_format(start_time), end=end_time_format(end_time))
...
class WeeklyTime:
    ...
    def __and__(self, other: "WeeklyTime") -> Optional["WeeklyTime"]:
        ...
        return result if result.days_of_week and result.start_time < result.end_time else None

    @classmethod
    def unlimited(cls) -> "WeeklyTimes":
        return WeeklyTime(DaysOfWeek.all(), time(0), time(0))
...
class WeeklyTimes(list[WeeklyTime]):
    ...
    def json(self):
        return dumps(
            [
                {
                    "daysOfWeek": [d.isoweekday() for d in wt.days_of_week],
    ...
    @property
    def min_start_time(self) -> str:
        return start_time_format(min(wt.start_time for wt in self))

    @property
    def max_end_time(self) -> str:
        return end_time_format(
            time(0) if any(wt.end_time == time(0) for wt in self) else max(wt.end_time for wt in self)
        )
...
class AbstractTime(StartEndMixin, models.Model):
    ...
    start: time = models.TimeField(_("start time"), blank=True, null=True)
    end: time = models.TimeField(
        _("end time"),
        blank=True,
        null=True,
    ...
    def __str__(self):
        if self.start is not None and self.end is not None:
            return _("{days}, {time}").format(
                days=self.days_of_week,
                time=time_slot_format(self.start, self.end),
            )
    ...
    def weekly_time(self) -> WeeklyTime:
        return WeeklyTime(
            ...
            start_time=time(0) if self.start is None else self.start,
            end_time=time(0) if self.end is None else self.end,
        )
    ...
    def get_next_time(self, now: date | datetime = None):
        ...
        force_next_day = isinstance(now, date) or self.start is None or self.start <= now.time()
        if force_next_day:
            now_day = (now_day + 1) % 7
