...
from leprikon.models.calendar import CalendarEvent, Resource, ResourceGroup
...
from .times import AbstractTime, TimesMixin, WeeklyTimes
...
class ActivityVariant(models.Model):
    ...
    def weekly_times(self) -> WeeklyTimes:
        collected_weekly_times = [WeeklyTimes(st.weekly_time for st in self.activity.times.all())] + [
            WeeklyTimes(a.weekly_time for a in resource.availabilities.all())
            for resource in self.required_resources.prefetch_related("availabilities")
        ]

        return reduce(
            (lambda a1, a2: a1 & a2),
            [wts for wts in collected_weekly_times if wts],
    ...
    def get_conflicting_timeslots(self, start_date: date, end_date: date):
        return get_conflicting_timeslots(
            start_date,
            end_date,
            self.required_resources.all(),
            self.required_resource_groups.all(),
        )
