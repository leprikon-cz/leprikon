...
urlpatterns = [
    ...
    url(  r'^{clubs}/$'.format(**LEPRIKON_URL), RedirectView.as_view(url='../', permanent=True)),
    d_url(r'^{clubs}/{mine}/$',                                                 'club_list_mine'),
    d_url(r'^{clubs}/{alternating}/$',                                          'club_alternating'),
    d_url(r'^{clubs}/(?P<club_type>[^/]+)/$',                                   'club_list'),
    d_url(r'^{clubs}/(?P<club_type>[^/]+)/(?P<pk>[0-9]+)/$',                    'club_detail'),
    d_url(r'^{clubs}/(?P<club_type>[^/]+)/(?P<pk>[0-9]+)/{registrations}/$',    'club_registrations'),
    d_url(r'^{clubs}/(?P<club_type>[^/]+)/(?P<pk>[0-9]+)/{journal}/$',          'club_journal'),
    d_url(r'^{clubs}/(?P<club_type>[^/]+)/(?P<pk>[0-9]+)/{edit}/$',             'club_update'),
    d_url(r'^{clubs}/(?P<club_type>[^/]+)/(?P<pk>[0-9]+)/{registration}/$',     'club_registration_form'),
